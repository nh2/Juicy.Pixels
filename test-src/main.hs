...
jpgParseECS_equivalence path = do
    ...
    let ecs =
            runGetOrFail (parseFramesStrictWithParseECSFunction JpgInternal.parseECS) bsl
    let ecs_simple =
            runGetOrFail (parseFramesStrictWithParseECSFunction JpgInternal.parseECS_simple) bsl
    when (ecs /= ecs_simple) $ do
        error "Test failure: parseECS /= parseECS_simple"
  where
    parseFramesStrictWithParseECSFunction :: Get L.ByteString -> Get [JpgInternal.JpgFrame]
    parseFramesStrictWithParseECSFunction parseECSFunction = do
        kind <- get
        ...
        remainingFrames <- JpgInternal.parseFramesStrict
